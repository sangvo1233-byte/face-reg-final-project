"""
Local Direct Detect V4.4 runner.
"""
...
class LocalDirectRunnerV4:
    def __init__(self):
        self._camera = get_camera()
        self._runtime: DetectV4RuntimeSession | None = None
    ...
    def start(self, session_id: int) -> dict:
        ...
        self._camera.start()
        self._thread = threading.Thread(target=self._loop, daemon=True, name="local-direct-scan-v4")
        self._thread.start()
        return {"success": True, "message": f"Local direct V4 scan started for session {session_id}", "state": "starting"}

    def stop(self) -> dict:
        with self._lock:
            self._stop_internal()
        return {"success": True, "message": "Local direct V4 scan stopped", "state": "stopped"}
    ...
    def _stop_internal(self):
        ...
        if self._runtime is not None:
            try:
                ...
                pass
            self._runtime = None
        self._hub.broadcast({"type": "scan_state", "status": "stopped", "message": "Local direct V4 scan stopped"})
    ...
    def _loop(self):
        try:
            self._runtime = DetectV4RuntimeSession(self._session_id)
        ...
        self._hub.broadcast({
            ...
            "status": "ready",
            "mode": "local_direct_v4",
            "message": "Local Direct V4 scan active",
            "session_id": self._session_id,
            "camera_status": self._camera.get_status(),
        })
        ...
        while self._running:
            ...
            try:
                ...
                logger.exception(f"Local direct V4 runtime error: {exc}")
...
_local_runner_v4: LocalDirectRunnerV4 | None = None


def get_local_runner_v4() -> LocalDirectRunnerV4:
    global _local_runner_v4
    if _local_runner_v4 is None:
        _local_runner_v4 = LocalDirectRunnerV4()
    return _local_runner_v4
