...
V4 keeps dev/detect-v3.py intact and adds:
  ...
  8. V4.4 portrait phone ROI + stronger phone rectangle sensitivity.
...
class CalibrationLogger:
    ...
    def write(self, payload: dict[str, Any]):
        ...
        safe = {
            ...
            "match_score": round(float(payload.get("confidence", 0.0)), 4),
...
def draw_info_panel(frame, results, challenge, debug_enabled, panel_w=INFO_PANEL_W):
    ...
    cv2.putText(panel, f"FACE RECOGNITION {DETECT_VERSION}", (S(14), S(28)), FONT, FS(0.55), C_GOLD, 1, cv2.LINE_AA)
    ...
    if challenge.active or challenge.message:
        cv2.putText(panel, f"ANTI-SPOOF {DETECT_VERSION}", (S(14), y), FONT, FS(0.45), C_ORANGE, 1, cv2.LINE_AA)
        y += S(22)
    ...
    for idx, r in enumerate(results[:4]):
        ...
        if r.get("aligned") is not None:
            ...
            if r.get("student_id"):
                cv2.putText(panel, f"ID: {r['student_id']}", (tx, y + S(56)), FONT_SMALL, FS(0.45), C_DIM, 1, cv2.LINE_AA)
            y += S(72)
        else:
            cv2.putText(panel, f"Name: {r.get('name', 'Unknown')}", (S(14), y), FONT_SMALL, FS(0.45), C_WHITE, 1, cv2.LINE_AA)
            y += S(20)

        conf_value = r.get("confidence", 0.0)
        ...
        if r.get("emb_dim"):
            ...
            if r.get("embedding_count"):
                ec = r["embedding_count"]
...
def update_challenge_status(
    *,
    ...
    if suspicious_reasons and not challenge.can_skip_for(match.student_id):
        ...
        result = challenge.update(match=match, live=live, rolling=rolling, metrics=metrics)
    ...
    *,
    ...
    if rolling.get("decision") == "block" or moire_decision == "block":
        ...
        suspicious_reasons = collect_suspicious_reasons(
            ...
            challenge_fail_count=challenge.fail_count,
        )
        status = update_challenge_status(
            challenge=challenge,
            match=match,
            candidate=candidate,
            live=live,
...
def main():
    ...
    print("=" * 72)
    print(f"  Detect {DETECT_VERSION} Research - Enhanced moire + context + portrait phone rectangle")
    print("=" * 72)
    ...
    win_name = f"Live Face Detection {DETECT_VERSION}"
            ...
            if now - last_detect_at >= detect_interval:
                ...
                try:
                    ...
                    for face in faces:
                        ...
                        cycle_moire_ms += float(entry.get("moire_ms", 0.0))
                        last_results.append(entry)
                    perf_moire_ms = cycle_moire_ms
